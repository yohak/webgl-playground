...
import frag from "../shader/three06h.frag";
import vert from "../shader/three06h.vert";
import { fillAttribute, getPointsOfFace } from "../utils/three-utils";
export type ThreeParticles06hProps = {};

export const ThreeParticles06h: FC<ThreeParticles06hProps> = ({}) => {
...
const init = (canvas: HTMLCanvasElement): (() => void) => {
  ...
  const render = () => {
    ...
    if (particleGeom) {
      ...
      for (let i = 0; i < 64; i++) {
        uniforms.uTouchPoints.value[i] = touchs[i]?.point || new THREE.Vector3(0, 0, 0);
        uniforms.uTouchPowers.value[i] = touchs[i] ? easing(touchs[i].power) : 0;
        uniforms.uTouchNormals.value[i] = touchs[i]?.normal || new THREE.Vector3(0, 0, 0);
      }
