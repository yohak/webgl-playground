...
export type ThreeParticles06fProps = {};

export const ThreeParticles06f: FC<ThreeParticles06fProps> = ({}) => {
...
const init = (canvas: HTMLCanvasElement): (() => void) => {
  ...
  const paths: string[] = ["/assets/heart1.obj", "/assets/woman1.obj"];
...
class ShapeVertices {
  ...
  updateTouching(intersections: THREE.Intersection[]) {
    intersections.forEach((intersection) => {
      ...
      if (touch) {
        touch.point = intersection.point.clone();
      } else {
        this.touching.push({
    ...
    this.touching.forEach((touch) => {
      if (intersections.find((intersection) => intersection.faceIndex === touch.faceIndex)) {
        touch.power = Math.min(touch.power + 0.05, 1);
      } else {
        touch.power -= 0.05;
      }
    });
  ...
  update(time: number, intersections: THREE.Intersection[]): number[] {
    ...
    for (let i = 0; i < originalPosition.array.length; i += 3) {
      ...
      //
      // const myTouching = this.touching.filter(
      //   (t) => t.point.distanceTo(currentPosition.clone()) <= minDistance
      // )[0];
      // if (myTouching) {
      //   const touchingDistance = myTouching.point.distanceTo(currentPosition.clone());
      //   touchingFactor =
      //     ((minDistance - touchingDistance) / minDistance + 1) * 20 * myTouching.power;
      // }
    }
