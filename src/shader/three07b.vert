...
struct Params {
  ...
  float noiseFreq;
  float touchWaveFreq;
  float touchWaveHeight;
  float touchWaveScale;
};
...
void main() {
  ...
  float waveScale = uParams.waveScale * 0.01;
  ...
  float touchWaveHeight = uParams.touchWaveHeight;
  float touchWaveScale = uParams.touchWaveScale * 0.01;

  float wy = radians(waveFreq * 360.);
  float noiseX = ((cnoise(vec4(x, y, z, noiseFreq)) * noiseHeight)) * pW;
  float noiseY = ((cnoise(vec4(y, z, x, noiseFreq)) * noiseHeight)) * pW;
  float noiseZ = ((cnoise(vec4(z, x, y, noiseFreq)) * noiseHeight)) * pW;
  float extraY = ((sin(wy + x * waveScale) * waveHeight) - (waveHeight * 0.5)) * pW;
  newPosition.y += extraY + noiseY;
  newPosition.x += noiseX;
  newPosition.z += noiseZ;
  //
  float wy2 = radians(touchWaveFreq * 360.);
  float touchExtraY = ((sin(wy2 + x * touchWaveScale) * touchWaveHeight) - (touchWaveHeight * 0.5)) * pT * pW;
  newPosition.y += touchExtraY;
  //  }

  if (uIsPoint && uMorphProgress > 0.){
    float x = mix(newPosition.x, targetPosition.x, uMorphProgress);
    float y = mix(newPosition.y, targetPosition.y, uMorphProgress);
    float z = mix(newPosition.z, targetPosition.z, uMorphProgress);
